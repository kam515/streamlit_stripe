    ...
    completion = client.beta.chat.completions.parse(
        ...
        messages=[
            {"role": "system", "content": system_message},
            {
                "role": "user",
                "content": (
                    f"Make a big picture outline of this goal with about 2-5 items: {prompty}"
                ),
            },
        ],
...
# ========== Build Dataframe for the first layer ==========
if st.session_state["project_dict"] is not None:
    layer = st.session_state["project_dict"]["outline_layers"][0]
    ...
    if ordered_data.empty:
        ...
        for idx, row in ordered_data.iterrows():
            ...
            with container:
                ...
                # Future sub-layer expansion
                see_detail_button = st.columns([10, 1])[1]
    ...
    # Sync final data back to session's project_dict
